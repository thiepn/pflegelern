...
test('frozen P26G bank and P27A release metadata are present in the published runtime', async ({ page, request }) => {
  await onboard(page);
  ...
  const audit = await (await request.get(`${BASE}reports/P27A_RELEASE_READINESS_AUDIT.json`)).json();
  const sw = await (await request.get(`${BASE}service-worker.js`)).text();
  expect(manifest.phase).toBe('P27A');
  expect(manifest.version).toBe('1.1.0-dev.27a');
  expect(manifest.status).toBe('p27a-release-readiness-audit');
  expect(audit.certifiedQuestionBank.intact).toBe(true);
  expect(audit.summary.staticRuntimeBlockers).toBe(0);
  expect(sw).toContain('pflegelern-p27a-v1.1.0-dev27a');
});

...
for (const [view, heading] of ROUTES) {
  test(`final route ${view} renders without runtime errors`, async ({ page }) => {
    await onboard(page);
...
test('frozen P26G bank and P27C final release metadata are present', async ({ page, request }) => {
  await onboard(page);
  ...
  const report = await (await request.get(`${BASE}reports/P27C_FINAL_RELEASE_CERTIFICATION.json`)).json();
  const sw = await (await request.get(`${BASE}service-worker.js`)).text();
  expect(manifest.phase).toBe('P27C');
  expect(manifest.version).toBe('1.1.0');
  expect(manifest.status).toBe('released');
  expect(report.certifiedQuestionBank.intact).toBe(true);
  expect(report.summary.failed).toBe(0);
  expect(report.summary.finalReleaseReady).toBe(true);
  expect(sw).toContain("const CACHE = 'pflegelern-v1.1.0';");
});
...
test('final PWA survives offline navigation reload after cache installation', async ({ page, context }) => {
